...
public class RetryExecutor {
    ...
    public <T> T execute(Supplier<T> action, String operationName) {
        ...
        log.error("All retry attempts failed for {}", operationName, lastException.getMessage());
    ...
    private boolean isCircuitBreakerOpen(Exception e) {
        ...
        if (e instanceof RetryableException re) {
            String message = re.getMessage();
            return message != null && message.toLowerCase().contains("circuit breaker open");
        }
    ...
    private boolean isRetryable(Exception e) {
        if (e instanceof ResponseStatusException responseStatusException) {
            ...
            return responseStatusException.getStatusCode().is5xxServerError()
                    && responseStatusException.getStatusCode().value() != 503;
        }

        if (e instanceof RetryableException retryableException) {
            String message = retryableException.getMessage();
            if (message != null && message.toLowerCase().contains("circuit breaker open")) {
                return false;
            }
            return true;
        }
    ...
    private ResponseStatusException mapFinalException(String operationName, Exception lastException) {
        ...
        if (lastException instanceof RetryableException re) {
            String message = re.getMessage();
